<template>
  <aside class="main-sidebar animated" :class="{ showSlide: sidebar.show, 
  hideSlide: !sidebar.show, expandSide:(!sidebar.collapsed||device.isMobile)}">
      ...
      <div class="sidebar">
        <el-menu :default-active="onRoutes"
          ...
          <template v-for="(item,index) in menuList">
            <sub-menu :param="item" :key="index"></sub-menu>
          </template>
        </el-menu>
      </div>
    </el-scrollbar>
    ...
    computed: {
      ...
      onRouteKeys(){
        const getParentArray = (path, ms, kas = []) => {
          ...
          console.log(kas.reverse());
          return kas.reverse();
        }
        console.log('路径列表：',this.$route.path)
