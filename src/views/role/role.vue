...
export default {
    ...
    methods:{
      ...
      renderContent(h, { node, data, store }){
        ...
        //       <i class="fa fa-trash" on-click={ () => this.deleteSelected(data.orgId) }></i>
        //     </span>
        //   </span>);
        return (
          <span>
      ...
      deleteSelected(orgId){
          console.log(11)
        this.$http.get(api.SYS_ROLE_DELETE + "?roleIds=" + orgId)
          .then(res => {
            this.$message('操作成功');
            this.deleteFromTree(this.roleTree, orgId);
            this.newAdd();
          }).catch(e =>{
          this.$message('操作成功');
          this.deleteFromTree(this.roleTree, orgId);
